...
- The loader preserves the CSV `description` and `diagnostics` fields instead of replacing them
  with values generated by the current decoder. This keeps existing exported metadata stable
  while semantic protocol fields are recalculated from the raw frame.
...
- Stored `description` and `diagnostics` survive loading unchanged.
...
| Sad path | Invalid rows still fail before replay; standard, CAN error, unsupported, and incomplete raw frames gain no unsupported semantic values |
| Edge cases | Empty captures remain valid; arbitrary `decoded_values` is ignored; persisted empty and non-empty metadata is preserved |
